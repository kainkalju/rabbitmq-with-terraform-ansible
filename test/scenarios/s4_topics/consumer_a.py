#!/usr/bin/env python3
"""
S4 Topics — consumer A.

Subscribes with pattern: *.critical
Receives: kern.critical, auth.critical
...
def on_message(
    ...
    try:
        ...
        print(
            f"[s4-consumer-A/{PATTERN}/{HOSTNAME}] received #{data.get('seq')}"
            f" key={data.get('key')}"
            f" ts={data.get('ts')}",
            flush=True,
        )
    except json.JSONDecodeError:
        print(f"[s4-consumer-A/{PATTERN}/{HOSTNAME}] received non-JSON: {body!r}", flush=True)
    ch.basic_ack(delivery_tag=method.delivery_tag)


def run() -> None:
    while True:
        connection = connect(HOSTS, USER, PASS, role=f"s4-consumer-A/{HOSTNAME}")
        try:
            ...
            print(
                f"[s4-consumer-A/{HOSTNAME}] bound to {EXCHANGE}"
            ...
            print(
                f"[s4-consumer-A/{HOSTNAME}] connection lost: {exc}, reconnecting",
                flush=True,
            )
            try:
                connection.close()
